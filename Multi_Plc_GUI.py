...
class WidgetsDemo:
    def __init__(self):
        ...
        btnCls = Button(frame2, text="清屏", command=self.clearMsg, width=8)
        ...
        self.IP = StringVar()
        self.IP.set('192.168.1.125')
        self.slot = IntVar()
        self.ddl = ttk.Combobox(frame2)
        self.DBType = ttk.Combobox(frame2)
        self.slot.set('0')
        self.rack = IntVar()
        current_path = os.path.abspath(os.path.dirname(__file__))
        with open(current_path + '/dir/' + 'plc_type.yml', 'r') as f:
            temp = yaml.load(f, Loader=yaml.FullLoader)
            for i in temp.values():
                a.append(i)
        self.ddl['value'] = a
        self.ddl.current(0)
        self.DBType['value'] = ["bool", "int", "string"]
        # self.DBType.current(0)
        self.rack = IntVar()
        self.rack.set('0')
        self.port = IntVar()
        self.port.set(502)
        ...
        lblDBType.grid(row=5, column=1)  #####
        self.DBType.grid(row=5, column=2)
    ...
    def write_yml(self):
        data = {
            ...
            'port': self.rack.get(),
            'db': self.DBNo.get(),
            'dataAd': self.dataAd.get(),
            'dataSz': self.dataSz.get(),
            'dataType': self.DBType.get(),
        }
        ...
        yamlpath = os.path.join(curpath, "data.yml")
    ...
    def testCnn(self):
        ...
        connection_result = container.get("A1").execution_connect()
        self.Text_Show(connection_result)
    ...
    def Text_Show(self,result):
        if isinstance(result, list):
            ...
            self.txtMsg.insert(END, result + '\n')
        elif isinstance(result, int):
            self.txtMsg.insert(END, str(result) + '\n')
        elif isinstance(result, tuple):
            self.txtMsg.insert(END, str(result[0]) + '\n')
